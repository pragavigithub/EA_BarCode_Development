...
class SAPIntegration:
    ...
    def create_purchase_delivery_note(self, grn_document):
        """Create Purchase Delivery Note in SAP B1 with exact JSON structure specified"""
        if not self.ensure_logged_in():
            # Return success for offline mode
            import random
        ...
        # Get PO data first to ensure proper field mapping
        po_data = self.get_purchase_order(grn_document.po_number)
        if not po_data:
            return {
                'success':
                False,
                'error':
                f'Purchase Order {grn_document.po_number} not found in SAP B1'
            }
        ...
        # Generate unique external reference number
        external_ref = self.generate_external_reference_number(grn_document)

        # Get first warehouse code from PO DocumentLines to determine BusinessPlaceID
        first_warehouse_code = None
        if grn_document.items:
            for item in grn_document.items:
                if item.qc_status == 'approved':
                    # Find matching PO line to get proper warehouse code
                    for po_line in po_data.get('DocumentLines', []):
                        if po_line.get('ItemCode') == item.item_code:
        ...
        for item in grn_document.items:
            ...
            if po_line_num is None:
                logging.warning(
                    f"PO line not found for item {item.item_code} in PO {grn_document.po_number}"
                )
                continue  # Skip items not found in PO
        ...
        pdn_data = {
            ...
            "DocDate": doc_date,
            "DocDueDate": doc_due_date,
            "Comments": grn_document.notes or "Auto-created from PO after QC",
            "NumAtCard": external_ref,
            "BPL_IDAssignedToInvoice": business_place_id,
            "DocumentLines": document_lines
        }
        ...
        try:
            ...
            if response.status_code == 201:
                result = response.json()
                logging.info(
                    f"Successfully created Purchase Delivery Note {result.get('DocNum')} for GRPO {grn_document.id}"
                )
                return {
                    'success':
                    True,
